...
from utils import get_settings, get_size, verifyRequest, save_group_settings, temp, verify_user, check_token, check_verification, get_token, send_all
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        return
    if AUTH_CHANNEL and not await verifyRequest(client, message):
        try:
...
async def shortlink(bot, message):
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and str(userid) not in ADMINS:
        ...
        pass
    try:
        command, Auth_Channel = data.split(" ")
    except:
